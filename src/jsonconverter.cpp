...
string convertServerInfo(void){
    ...
    json_object_set_new(json, "title", json_string("HTTP Server"));
    json_object_set_new(json, "developer",json_string( "Egor Schupakovskiy"));
    //get current time
...
string convertFileContent(string filePath){
    ...
    int i = 0;
    while(fileContent[i]){
        if(isdigit(fileContent[i])) digit++;
        i++;
    }
    json_t * json = json_object();
    json_object_set_new(json, "FileName", json_string( "data.txt"));
    json_object_set_new(json, "digit",json_integer(digit));
    char * str = json_dumps(json, JSON_INDENT(2) | JSON_PRESERVE_ORDER);
    string jsonString(str);
    free(str);
    json_decref(json);
